...
public class PasswordConstraintValidator implements ConstraintValidator<Password, String> {
    ...
    public boolean isValid(String value, ConstraintValidatorContext context) {
        ...
        if (this.password.minLength() > this.password.maxLength()) {
            addViolation(PASSWORD_MIN_MAX_LENGTH_MISMATCH);
            violationsCount++;
        }

        if (this.value.length() < this.password.minLength()) {
            addViolation(String.format(PASSWORD_TOO_SHORT, this.password.minLength()));
            violationsCount++;
        }

        if (this.value.length() > this.password.maxLength()) {
            addViolation(String.format(PASSWORD_TOO_LONG, this.password.maxLength()));
            violationsCount++;
        }

        if (!doesContainDigit()) {
            violationsCount++;
            addViolation(PASSWORD_NO_DIGIT);
        }

        if (!doesContainLowercase()) {
            violationsCount++;
            addViolation(PASSWORD_NO_LOWERCASE);
        }

        if (!doesContainUppercase()) {
            violationsCount++;
            addViolation(PASSWORD_NO_UPPERCASE);
        }

        if (!doesContainSpecialSymbol()) {
            violationsCount++;
            addViolation(PASSWORD_NO_SPECIAL_SYMBOL);
        }
